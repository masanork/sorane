...
sorane は `type: dataset` の frontmatter で **データセットごとのライセンス**（`license:`、SPDX 推奨）を宣言する仕組みを持ちます。これは **利用者が sorane で公開するデータ**向けであり、sorane 本体や本ドキュメントの MIT とは別レイヤーです。官公庁データでは [公共データ利用規約](https://www.digital.go.jp/resources/open_data) など、データの性質に合ったライセンスを dataset に記載してください。詳細は [OKF プロファイル](okf-profile.html) と [Open Data Portal 連携](open-data-harvesting.html) を参照してください。