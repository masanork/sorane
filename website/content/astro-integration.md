...
- ネイティブ Rust backend の validation: Phase A（OKF Schema、見出し、content-quality）+ Phase B（FAQ / glossary / directory index ヒント）+ Phase C（reference / dataset、lang mixing、term links、revision、redirect frontmatter、unsafe links）。フル i18n 横断・`sorane.yaml` redirects・diagram enabled 時の alt は未移植です。backend contract の `site.lang`（既定 `ja`）で言語混在ゲートの既定言語を渡せます。
