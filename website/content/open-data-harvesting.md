...
sorane の静的サイトは **単一オリジンのデータカタログ** として機能します。全国ポータル（[data.europa.eu](https://data.europa.eu)）や CKAN インスタンスへ載せるには、ビルド成果物を **外部ハーベスターが取り込める形** にしておきます。
...
[data.europa.eu](https://data.europa.eu) のハーベストは **DCAT-AP カタログフィード** を前提とします。sorane 単体はポータル準拠の RDF ストアではありませんが、`catalog-dcat.jsonld` とランディングページで次の項目を満たしやすくします。
