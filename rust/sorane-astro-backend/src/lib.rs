...
pub fn run_backend(input: BackendInput) -> Result<BackendOutput, String> {
...
/// JSON stdin/stdout contract used by the CLI and WASM bindings.
pub fn run_backend_json(input_json: &str) -> Result<String, String> {
    let input: BackendInput =
        serde_json::from_str(input_json).map_err(|err| format!("invalid input JSON: {err}"))?;
    let output = run_backend(input)?;
...
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen]
pub fn run_sorane_astro_backend(input_json: &str) -> Result<String, JsValue> {
    run_backend_json(input_json).map_err(|e| JsValue::from_str(&e))
}