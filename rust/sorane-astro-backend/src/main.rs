...
fn build_llms(site: &BackendSite, concepts: usize) -> String {
    format!(
        "# {}\n\n> {}\n\n## Astro integration\n\nGenerated by `sorane-astro-backend` CLI.\n\n## Labeled articles\n\nLabeled articles: {}\n",
        site.title, site.description, concepts
    )
}
...
fn run_backend(input: BackendInput) -> Result<BackendOutput, String> {
    ...
    if outputs.llms_txt {
        artifacts.push(BackendArtifact {
            ...
            kind: "text".to_string(),
            content: build_llms(&input.site, concepts.len()),
        });
    }
