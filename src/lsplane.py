...
def print_plane_ranges_as_hex(sorted_codepoints):
    # 最初のコードポイントで開始
    current_plane = sorted_codepoints[0] // 0x10000
    start = sorted_codepoints[0]
    end = start

    for cp in sorted_codepoints[1:]:
        # 現在のコードポイントが同じプレーンにあるかチェック
        if cp // 0x10000 == current_plane:
            end = cp
        else:
...
def main():
    ...
    print_plane_ranges_as_hex(sorted(codepoints))
